...
agent-skills what-if-analysis scenario --symbol calculate_score

# 3. Parse exact AST call sites in a specific source file
agent-skills what-if-analysis ast --symbol calculate_score --file src/core.rs

# 4. Auto-generate adversarial RED counterfactual unit test
agent-skills what-if-analysis counterfactual --symbol calculate_score

# 5. Intercept pre-emptive failure risks before committing
agent-skills what-if-analysis preempt --symbol calculate_score
```
...
agent-skills what-if-analysis impact --symbol parse_simple_yaml --json
```
