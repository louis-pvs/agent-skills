...
if ! command -v node >/dev/null 2>&1; then
  ...
  echo "Antigravity/Gemini plugins cannot bundle or auto-install Node." >&2
  echo "" >&2
...
in_host_agent_context() {
  ...
  case "$SCRIPT_DIR" in
    */.codex/skills/*|*/.claude/skills/*|*/.agents/skills/*|*/.gemini/config/skills/*)
...
# Host agents (Codex CLI / Claude Code / Antigravity) cannot update native TODO/plan UIs while a long-running
# command is executing. If we're in a host agent context, return immediately with a single `wait`
# JSON payload (includes `.ui.codex.update_plan.plan` / `.ui.claude.todo_write.todos` / `.ui.gemini.todo_write.todos`) and let the
