...
const WORKER_PATH = path.join(SCRIPT_DIR, 'council-job-worker.cjs');
...
function detectHostRole() {
  ...
  if (normalized.includes('/.agents/skills/') || normalized.includes('/.gemini/')) {return 'gemini';}
  return 'unknown';
}
...
function resolveAutoRole(role, hostRole) {
  ...
  if (hostRole === 'gemini') {return 'gemini';}
  ...
  try {
    ...
    exitWithError(
      [
        ...
        'Please run pnpm install yaml or configure your package.json.',
      ].join('\n')
    );
  }
...
function buildCouncilUiPayload(statusPayload) {
  ...
  return {
    ...
    gemini: { todo_write: { todos: claudeTodos } },
  };
}
...
Usage:
  council-job.sh start [--config path] [--chairman auto|claude|codex|gemini|...] [--jobs-dir path] [--json] "question"
